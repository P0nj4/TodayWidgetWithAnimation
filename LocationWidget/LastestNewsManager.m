...
- (void)loadLastestNewsWithSuccessCompletition:(void (^)(NSDictionary * result))onSuccess onFailCompletition:(void (^)(NSError * error))onFail {
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:[containerURL path]]) {
        self.dicLastestNews = [NSKeyedUnarchiver unarchiveObjectWithData:[NSData dataWithContentsOfURL:containerURL]];
    }
        ...
        if (error) {
            ...
            if (error || !json) {
                ...
                if (self.dicLastestNews) {
                    ...
                    NSData *serializedData= [NSKeyedArchiver archivedDataWithRootObject:self.dicLastestNews];
                    [serializedData writeToURL:containerURL atomically:YES];
                }
                onSuccess(self.dicLastestNews);
            }
        }
